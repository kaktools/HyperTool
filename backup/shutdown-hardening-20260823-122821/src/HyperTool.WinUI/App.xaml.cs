...
{
    private const string CurrentHostWhatsNewNoticeVersion = "2026-08-v271-shutdown-hardening";
    private const int HostSessionEndingCleanupTimeoutSeconds = 22;
        ...
        string configPath)
    {
        ...
        try
        {
            ...
            var infoLines = string.Join("\n", new[]
            {
                ...
                string.Empty,
                "Wichtige Hinweise:",
                "- Windows-Shutdown wurde für Host+Guest gehärtet: USB-Cleanup startet früher und läuft mit erweitertem Zeitbudget.",
                "- USB-Detach/Unshare beim Herunterfahren bleibt im Safe-Mode (ohne späte Elevation-/Service-Starts), um PowerShell-/sc.exe-Fehler zu vermeiden.",
                "- SessionEnding blockt Windows jetzt kurz länger, damit aktive USB-Verbindungen sauber gelöst und für andere VMs wieder frei werden."
            });
    ...
    private async Task ExecuteUsbShutdownCleanupAsync(bool forceCleanupDuringSessionEnding = false)
    {
        ...
        var sessionEndingActive = Interlocked.CompareExchange(ref _sessionEndingExitStarted, 0, 0) == 1
            || Environment.HasShutdownStarted
            || AppDomain.CurrentDomain.IsFinalizingForUnload();

        if (sessionEndingActive)
        {
        ...
        if (!_mainViewModel.UsbUnshareOnExit && !forceCleanupDuringSessionEnding)
        {
        ...
        if (!_mainViewModel.UsbUnshareOnExit && forceCleanupDuringSessionEnding)
        {
            Log.Information("USB cleanup on shutdown is forced during SessionEnding although UsbUnshareOnExit=false.");
        }

        try
        {
            var detachOnly = sessionEndingActive;
    ...
    private void OnSystemSessionEnding(object? sender, SessionEndingEventArgs e)
    {
        ...
        try
        {
            _mainViewModel?.SetUsbShutdownCleanupMode(true);
        }
        catch
        {
        }
        ...
        try
        {
            var cleanupTask = Task.Run(() => ExecuteUsbShutdownCleanupAsync(forceCleanupDuringSessionEnding: true));
            if (!cleanupTask.Wait(TimeSpan.FromSeconds(HostSessionEndingCleanupTimeoutSeconds)))
            {
                Log.Warning("USB cleanup during SessionEnding reached timeout window. TimeoutSeconds={TimeoutSeconds}", HostSessionEndingCleanupTimeoutSeconds);
            }
        }
        catch (Exception ex)
        {
        ...
        finally
        {
            try
            {
                _mainViewModel?.SetUsbShutdownCleanupMode(false);
            }
            catch
            {
            }
