...
- Shutdown-Verhalten Host + Guest beim direkten Windows-Herunterfahren gehärtet:
	- SessionEnding-USB-Cleanup läuft mit erweitertem Zeitbudget, damit Detach/Unshare zuverlässiger abgeschlossen werden.
	- USB-Cleanup wird während SessionEnding im sicheren Shutdown-Modus ausgeführt (keine späten Elevation-/Service-Starts), wodurch PowerShell-/`sc.exe`-Fehler beim Herunterfahren reduziert werden.
	- Guest erzwingt den finalen USB-Disconnect im SessionEnding-Pfad, damit Geräte nach dem Shutdown wieder schneller für andere VMs attachbar sind.
- Hinweisdialoge "Was ist neu" für Host und Guest auf den Stand `v2.7.1` mit Shutdown-/USB-Stabilitätsfokus angehoben.
