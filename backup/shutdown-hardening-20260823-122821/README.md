...
- Version: **v2.7.1**
- Windows-Shutdown für Host und Guest verbessert: SessionEnding hält HyperTool jetzt länger aktiv, damit USB-Detach/Unshare sauber abgeschlossen werden kann.
- USB-Cleanup beim Herunterfahren läuft im sicheren Shutdown-Modus ohne späte Elevation-/Service-Starts; dadurch treten PowerShell-/sc.exe-Fehler beim Windows-Exit seltener auf.
- Geräte werden beim direkten Windows-Herunterfahren zuverlässiger freigegeben und sind danach schneller wieder für Attach in anderen VMs verfügbar.
